...
void __fastcall TFSearch::VIZUALIZAREClick(TObject *Sender)
{
	...
	if (StartNrDos->Text != "" || FinishNrDos->Text != "" || Data->Checked == true)
	{
		...
		free_query += "(SELECT NPOD.NUME FROM NPP_ORDONATOR NPOD INNER JOIN ORDONATOR OD ON NPOD.NPP_ORDONATOR_ID=OD.NPP_ORDONATOR_ID ";
		free_query += "WHERE D.ORDONATOR_ID=OD.ORDONATOR_ID) AS NUME_ORDONATOR, ";
		free_query += "(SELECT NPOD.PRENUME FROM NPP_ORDONATOR NPOD INNER JOIN ORDONATOR OD ON NPOD.NPP_ORDONATOR_ID=OD.NPP_ORDONATOR_ID ";
		free_query += "WHERE D.ORDONATOR_ID=OD.ORDONATOR_ID) AS PRENUME_ORDONATOR, ";
		...
		free_query += "(SELECT DIRECTIE_ID FROM ORDONATOR OD WHERE D.ORDONATOR_ID=OD.ORDONATOR_ID) AS DIRECTIE_ID, ";
		free_query += "(SELECT NPP_ORDONATOR_ID FROM ORDONATOR OD WHERE D.ORDONATOR_ID=OD.ORDONATOR_ID) AS NPP_ORDONATOR_ID, ";
		...
		free_query += "(SELECT SUBDIVIZIUNE_ID FROM ORDONATOR OD INNER JOIN DIRECTIE DR ON OD.DIRECTIE_ID=DR.DIRECTIE_ID ";
		free_query += "WHERE D.ORDONATOR_ID=OD.ORDONATOR_ID) AS SUBDIVIZIUNE_ID, ";
		...
		free_query += "(SELECT TIP_RAPORT_ID FROM RAPORT RA WHERE RA.RAPORT_ID=D.RAPORT_ID) AS TIP_RAPORT_ID, ";
	...
	else
	{
		...
		free_query += "( SELECT GEN_EXAMINARE FROM GEN_EXAMINARE GE INNER JOIN RAPORT RA ON GE.GEN_EXAMINARE_ID=RA.GEN_EXAMINARE_ID ";
		free_query += "WHERE RA.RAPORT_ID=D.RAPORT_ID) AS GEN_EXAMINARE ";
		free_query += "FROM DOSAR D ) AS TOT ORDER BY TOT.NR_DOSAR ASC";
...
void __fastcall TFSearch::AdaugaRaport1Click(TObject *Sender)
{
	FRaport->ShowModal();
}
//---------------------------------------------------------------------------
